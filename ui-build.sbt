...
// Execute frontend build task prior to play test execution. This enables e2e testing.
test := ((test in Test) dependsOn `ui-test`).value

lazy val ITest = config("it") extend (Test)
IntegrationTest / test := ((IntegrationTest / test) dependsOn `ui-prod-build`).value
IntegrationTest / testOnly := ((testOnly in IntegrationTest) dependsOn `ui-prod-build`).evaluated
IntegrationTest / testQuick := ((testQuick in IntegrationTest) dependsOn `ui-prod-build`).evaluated
